...
| F-1236 | high | Supply snapshots can be stamped at a fresh ledger while using stale component observations | Supply refreshers; supply observer storage; asset supply API/market-cap fields | XFI-0028; EV-0047; EV-0106; EV-0108; EV-0109; EV-0110; EV-0111; EV-0112 | open | supply/data-quality | Wave 18 commits both classic and SEP41 storage freshness producers, but XLM freshness production remains absent and freshness-query failures still degrade to the zero-value bypass. |
...
Current-head reconciliation: settled `HEAD=fb0b3073...` now commits both storage-backed producer paths added during the supply remediation wave. `timescale.Store.MinClassicComponentLedger` feeds `ClassicSupplyComponents.MinComponentLedger`, `timescale.Store.MinSEP41ComponentLedger` feeds `SEP41SupplyComponents.MinComponentLedger`, and the targeted supply/timescale/aggregator/ops test set is green again. That remains non-closing. XLM still has no freshness producer at all, and both storage readers deliberately turn freshness-query failures into `0`, which explicitly re-enters the refresher's permissive bypass. The code is materially closer, but the original fresh-ledger/stale-component risk is not fully removed across every supply surface.
