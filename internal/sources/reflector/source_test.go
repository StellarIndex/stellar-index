...
func TestDecodeUpdate_fanout(t *testing.T) {
	...
	if updates[0].Timestamp.Unix() != 1745000000 {
		t.Errorf("timestamp wrong: %v", updates[0].Timestamp)
	}
...
func TestSource_BasicsByVariant(t *testing.T) {
	cases := []struct {
		name     string
		fn       func(*stellarrpc.Client, string, ...Option) *Source
		wantName string
	}{
		{"dex", NewDEX, SourceDEX},
		{"cex", NewCEX, SourceCEX},
		{"fx", NewFX, SourceFX},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if s.decimals != DefaultDecimals {
				t.Errorf("default decimals = %d", s.decimals)
			}
		})
	}

	// Option-based override.
	s := NewDEX(nil, "Ctest", WithDecimals(10), WithPollInterval(500*time.Millisecond))
	if s.decimals != 10 || s.pollInterval != 500*time.Millisecond {
