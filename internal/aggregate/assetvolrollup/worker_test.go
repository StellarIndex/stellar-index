...
func (f *fakeRefresher) RefreshAssetVolume24h(context.Context) error {
	f.calls++
	return f.err
}
...
func TestRefresh_Metrics(t *testing.T) {
	...
	okBefore := obstest.HistogramSampleCount(t,
		obs.AssetVolumeRollupSweepDurationSeconds, "outcome", "ok")
	w := New(&fakeRefresher{}, Options{})
	w.refresh(ctx)
	if got := obstest.HistogramSampleCount(t,
		obs.AssetVolumeRollupSweepDurationSeconds, "outcome", "ok"); got != okBefore+1 {
		t.Errorf("ok histogram count = %d, want %d", got, okBefore+1)
	}

	errBefore := obstest.HistogramSampleCount(t,
		obs.AssetVolumeRollupSweepDurationSeconds, "outcome", "refresh_error")
	wErr := New(&fakeRefresher{err: errors.New("pg down")}, Options{})
	wErr.refresh(ctx)
	if got := obstest.HistogramSampleCount(t,
		obs.AssetVolumeRollupSweepDurationSeconds, "outcome", "refresh_error"); got != errBefore+1 {
