...
# Build + runtime image for ratesengine-ops.
...
RUN CGO_ENABLED=0 GOOS=linux \
    go build -trimpath -buildvcs=true \
      ...
      -o /out/ratesengine-ops \
      ./cmd/ratesengine-ops

FROM gcr.io/distroless/static-debian12:nonroot
COPY --from=builder /out/ratesengine-ops /usr/local/bin/ratesengine-ops
USER nonroot:nonroot
# no listening port
ENTRYPOINT ["/usr/local/bin/ratesengine-ops"]
