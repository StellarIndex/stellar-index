...
# Build + runtime image for ratesengine-indexer.
...
RUN CGO_ENABLED=0 GOOS=linux \
    go build -trimpath -buildvcs=true \
      ...
      -o /out/ratesengine-indexer \
      ./cmd/ratesengine-indexer

FROM gcr.io/distroless/static-debian12:nonroot
COPY --from=builder /out/ratesengine-indexer /usr/local/bin/ratesengine-indexer
USER nonroot:nonroot
EXPOSE 9464
ENTRYPOINT ["/usr/local/bin/ratesengine-indexer"]
